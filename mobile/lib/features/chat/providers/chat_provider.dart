...
class ChatState {
  ...
  List<ChatMessage> messagesFor(String conversationId) => messagesByConversation[conversationId] ?? const [];

  Set<String> typingIn(String conversationId) => typingByConversation[conversationId] ?? const {};

  bool hasMore(String conversationId) => hasMoreByConversation[conversationId] ?? true;
...
class ChatNotifier extends Notifier<ChatState> {
  ...
  void setConversations(List<Conversation> conversations) {
    state = state.copyWith(conversations: conversations, conversationsLoaded: true);
  }

  void upsertConversation(Conversation conversation) {
    final others = state.conversations.where((c) => c.id != conversation.id).toList();
    state = state.copyWith(conversations: [conversation, ...others]);
  }

  void touchConversationLastMessage(String conversationId, ChatMessage message) {
  ...
  void setMessages(String conversationId, List<ChatMessage> messages) {
    state = state.copyWith(messagesByConversation: {...state.messagesByConversation, conversationId: messages});
  }

  void prependMessages(String conversationId, List<ChatMessage> older) {
    final existing = state.messagesFor(conversationId);
    state = state.copyWith(messagesByConversation: {
  ...
  void setMessageStatus(String conversationId, String messageId, String userId, String status) {
    final messages = state.messagesFor(conversationId);
    state = state.copyWith(messagesByConversation: {
      ...state.messagesByConversation,
      conversationId: messages.map((m) => m.id == messageId ? m.withStatus(userId, status) : m).toList(),
    });
  }
  ...
  void setTyping(String conversationId, String userId, bool isTyping) {
    ...
    state = state.copyWith(typingByConversation: {...state.typingByConversation, conversationId: current});
  }

  void setHasMore(String conversationId, bool hasMore) {
    state = state.copyWith(hasMoreByConversation: {...state.hasMoreByConversation, conversationId: hasMore});
  }
}
