...
class AuthActions {
  ...
  Future<void> _applyAuth(Map<String, dynamic> data) async {
    ...
    _ref.read(authSessionProvider.notifier).setSession(
          user: user,
          accessToken: accessToken,
          refreshToken: refreshToken,
        );
    await _ref.read(storageServiceProvider).saveAuth(
          AuthBlob(user: user, accessToken: accessToken, refreshToken: refreshToken),
        );
  ...
  Future<void> register({
    ...
    final data = await _ref.read(authApiProvider).register(
          name: name,
          username: username,
          password: password,
          email: email,
  ...
  Future<bool> checkUsername(String username) => _ref.read(authApiProvider).checkUsername(username);
  ...
  Future<void> verifyOtp({required String phone, required String code}) async {
    final data = await _ref.read(authApiProvider).verifyOtp(phone: phone, code: code);
    await _applyAuth(data);
  }
  ...
  Future<void> updateUser(Map<String, dynamic> partial) async {
    ...
    if (session.user == null || session.accessToken == null || session.refreshToken == null) return;
    await _ref.read(storageServiceProvider).saveAuth(
          AuthBlob(user: session.user, accessToken: session.accessToken, refreshToken: session.refreshToken),
        );
  }
