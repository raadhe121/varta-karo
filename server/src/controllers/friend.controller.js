...
export async function sendFriendRequest(req, res) {
  ...
  if (existing) {
    return res.status(409).json({ message: 'A friend request already exists', request: existing });
  }

  const request = await FriendRequest.create({ requesterId: req.userId, addresseeId, status: 'pending' });
  await createNotification(
    { recipientId: addresseeId, actorId: req.userId, type: 'friend_request' },
    req.app.get('io')
  );
...
export async function acceptFriendRequest(req, res) {
  ...
  await request.save();
  await createNotification(
    { recipientId: request.requesterId, actorId: req.userId, type: 'friend_accepted' },
    req.app.get('io')
  );
...
export async function declineFriendRequest(req, res) {
  ...
  if (!request) return res.status(404).json({ message: 'Request not found' });
  if (request.addresseeId !== req.userId) {
    return res.status(403).json({ message: 'Only the addressee can decline this request' });
  }
...
export async function listIncomingFriendRequests(req, res) {
